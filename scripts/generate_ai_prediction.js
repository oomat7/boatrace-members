...
async function main() {
  ...
  for (const race of races) {
    ...
    // 結論用のラベル（名前があれば付ける）
    const label1 = n1 ? `1号艇 ${n1}` : '1号艇（選手名不明）';
    const label2 = n2 ? `2号艇 ${n2}` : '2号艇（選手名不明）';
    const label3 = n3 ? `3号艇 ${n3}` : '3号艇（選手名不明）';
    const label4 = n4 ? `4号艇 ${n4}` : '4号艇（選手名不明）';
    const label5 = n5 ? `5号艇 ${n5}` : '5号艇（選手名不明）';
    const label6 = n6 ? `6号艇 ${n6}` : '6号艇（選手名不明）';
...
【守ってほしいこと】
- 買い目は必ず「${race.picks || '未設定'}」の範囲内でコメントしてください。新しい組み合わせは増やさないでください。
- 選手名と艇番もできるだけ本文の中で触れてください（例:「1号艇 ${n1 || '○○'} はイン戦安定」など）。
- モーターや足色のコメントは「一般的な傾向」としての表現にとどめてください。

【出力フォーマット（この形を目安にしてください）】
...
▲単穴：${label3}
☆ヒモ穴：${label4}
※必要であれば5号艇 ${n5 || '（選手名不明）'}、6号艇 ${n6 || '（選手名不明）'} にも触れてください。
